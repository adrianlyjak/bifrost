...
func ConvertBifrostMessagesToOpenAIMessages(messages []schemas.ChatMessage) []OpenAIMessage {
	...
	for i, message := range messages {
		...
		// call_id as "<baseID>_ts_<sig>") from the tool result's tool_call_id; OpenAI rejects
		// IDs over 64 chars. Clone first — ChatToolMessage is shared with the caller's input.
		...
		if message.ChatAssistantMessage != nil {
			// Strip the same embedded signature from assistant tool call IDs. Clone the slice
			// only when a strip is actually needed so the caller's input is never mutated.
			toolCalls := message.ChatAssistantMessage.ToolCalls
			needsStrip := false
			for j := range toolCalls {
				if toolCalls[j].ID != nil && strings.Contains(*toolCalls[j].ID, utils.ThoughtSignatureSeparator) {
					needsStrip = true
					break
				}
			}
