		...
		for j, existingVirtualKey := range governanceConfig.VirtualKeys {
			...
			if idMatch || nameMatch {
				...
				if forceFileSync || existingVirtualKey.ConfigHash != fileVKHash {
					...
					if configData.Governance.VirtualKeys[i].Value.IsFromSecret() {
						if configData.Governance.VirtualKeys[i].Value.GetValue() == "" {
						...
						// env/vault ref is valid — store it as-is without prefix validation
					} else {
